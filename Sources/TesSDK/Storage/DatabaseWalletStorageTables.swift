//
//  DatabaseWalletStorageTables.swift
//  TesSDK
//
...
struct AddressDBModel {
    ...
    func toAddress() throws -> Address {
        return try Address(
            index: UInt32(getIndex()),
            address: EthereumAddress(rawAddress: Data(getAddress().bytes)),
            network: getNetwork()
        )
    }
    ...
    static func setters(storage: Address, accountId: String) throws -> [Setter] {
        return [
            ...
            AddressDBModel.address <- Blob(bytes: storage.address.rawValue.bytes),
